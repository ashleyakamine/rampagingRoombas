...
class GoToGoalNode(Node):
    def __init__(self):
        ...
        self.path = []
        self.got_goal = False
        self.status_array = GoalStatusArray()
        ...
        # Subscribe to goal 
        self.goal_sub = self.create_subscription(PoseStamped, '/goal_pose', self.callback_goal, 10)

        self.timer = self.create_timer(5, self.execute_callback)
        
        # Publisher for goal status
        self.goal_status_pub = self.create_publisher(GoalStatusArray, '/goal_status', 10)
    ...
    def callback_goal(self, msg):
        ...
        if self.greedy_path(goal_x, goal_y):
            status.goal_id = 3
            self.status_array.append(status)
            self.goal_status_pub.publish()

            # only run it goal X & y are new
